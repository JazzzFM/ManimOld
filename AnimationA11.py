...
class Uno(GraphScene):
    ...
    def construct(self):
        ...
        V1_x = Arrow(
                origin-0.15*np.array([1,0,0]),
                origin + np.array([5,0,0]),
                color = BLUE)

        V1_y = Arrow(
                V1_x.get_end() -0.25*np.array([-0.2,1,0]),
                V1_x.get_end() + np.array([0.25*0.2, 4.1, 0]),
                color = GREEN
                )
...
class Dos(GraphScene, MovingCameraScene):
    def construct(self):
        ...
        r_t = TexMobject("\\vec{r}").move_to(
                r.get_center() + np.array([-0.5, 0.5, 0]))
        
        r_b = Arrow(0.92*np.array([3,2,0]), 1.5*np.array([3,2,0]))
        t_b = Arrow(np.array([3.1,1.75,0]), np.array([2,3.5,0]))
        
        teta = TexMobject("\\theta").move_to(np.array([1.9, 0.4, 0]))
        ...
        r_tc = TexMobject("\\vec{r}").move_to(
        rc.get_center() + np.array([-0.5, 0.5, 0]))
        ...
        for i in range(35, 385, 5):
            v_cords.append(np.sqrt(13)*np.array([np.cos(PI*i/180), 
                np.sin(PI*i/180), 0]))
            v_cords2.append(np.sqrt(12.5)*np.array([np.cos(PI*(i-4)/180),
                np.sin(PI*(i-4)/180), 0]))
            t_cords.append(np.sqrt(15.7)*np.array([np.cos(PI*(i+28)/180),
                np.sin(PI*(i+28)/180), 0]) )
            t_cords2.append(np.sqrt(15.5)*np.array([np.cos(PI*(i+28)/180),
                np.sin(PI*(i+28)/180), 0]) )
        ...
        dist_1 = np.sqrt(
        ((0.9*np.sqrt(13)*np.cos(PI*1/180))-1.5*np.sqrt(13)*np.cos(PI*1/180))**2+(0.9*np.sin(PI*1/180)-1.5*np.sin(PI*1/180))**2)
        
        for i in range(0, len(v_cords)):
            v_arrows.append(
                    Vector(0.98*v_cords[i], tip_length = 0.2).set_color(RED))
            r_arrows.append(
                    Arrow(0.9*v_cords[i], 1.5*v_cords[i], tip_length = 0.2) )
            t_arrows.append(
                    Arrow(v_cords2[i], t_cords[i], tip_length = 0.2) )
        
        for i in range(0, len(v_arrows)):
            v_tex.append(
                TexMobject("\\vec{r}").move_to(
                v_arrows[i].get_center() + np.array([-0.5, 0.2, 0])))
            r_tex.append(TexMobject("\\hat{r}").move_to(
                1.5*v_arrows[i].get_end()) )
        ...
        i_t = TexMobject("\\hat{i}").move_to(
                I.get_end() + np.array([0.2,  0.0, 0.0]))
        i_t.set_color(YELLOW)
        i_t.scale(0.4)
        
        j_t = TexMobject("\\hat{j}").move_to(
                J.get_end() + np.array([ 0.0, 0.2, 0.0]))
        ...
        py = DashedLine(
                1.43*v_cords[ult] - np.array([0, 0.5, 0]),
                1.43*v_cords[ult])
        ...
        px = DashedLine(
                1.43*v_cords[ult] - np.array([1.5, 0, 0]),
                1.43*v_cords[ult])

        px.set_color(BLUE)

        Lx = Line(
                1.43*v_cords[ult] - np.array([1.55, 0.56, 0]),
                1.43*v_cords[ult] - np.array([0, 0.56, 0])
                ).set_color(GREEN)

        Ly = Line(
        1.43*v_cords[ult] - np.array([1.54, 0.53, 0]),
        1.43*v_cords[ult] - np.array([1.54, 0.53, 0]) + np.array([0, 0.55, 0])
        ).set_color(BLUE)


        # -------------------------------------------------------------

        tetilla_1 = TexMobject("\\theta").move_to(
        Lx.get_center() + np.array([0, 0.13, 0]))

        tetilla_2 = tetilla_1.copy().move_to(
        Ly.get_center() + np.array([-0.1, 0.3, 0]))
        ...
        CA = TexMobject("C.A.").set_color(BLUE)
        CA.scale(0.28)
        CA.move_to(Lx.get_center() + np.array([0.15, 0.7, 0]))

        CO = TexMobject("C.O.").set_color(GREEN)
        CO.scale(0.28)
        CO.move_to(Ly.get_center() + np.array([1.8, 0, 0]))

        CA2 =  TexMobject("\\frac{C.A.}{1} = cos(\\theta)").set_color(BLUE)
        CA2.scale(0.28)
        CA2.move_to(Lx.get_center() + np.array([0.15, 0.75, 0]))
 
        CO2 = TexMobject("\\frac{C.O.}{1} = sen(\\theta)").set_color(GREEN)
        CO2.scale(0.28)
        CO2.move_to(Ly.get_center() + np.array([2.05, 0, 0]))

        self.play(
            Write(Lx), Write(CA), Write(px)
        )
        
        self.play(
            Write(Ly), Write(CO), Write(py)
        )
        ...
        self.play(
            ...
            ReplacementTransform(CO, CO2)
        )

        self.wait()
        
        CA3 =  TexMobject("C.A. = cos(\\theta)").set_color(BLUE)
        CA3.scale(0.28)
        CA3.move_to(Lx.get_center() + np.array([0.15, 0.7, 0]))

        CO3 = TexMobject("C.O. = sen(\\theta)").set_color(GREEN)
        CO3.scale(0.28)
        CO3.move_to(Ly.get_center() + np.array([2.0, 0, 0]))
 
        self.play(
            ReplacementTransform(CA2, CA3),
            ReplacementTransform(CO2, CO3)
        )
        ...
        self.play(
            ReplacementTransform(CA3, cos),
            ReplacementTransform(CO3, sen)
        )
       ...
       # -------------------------------------------------------------
        ...
        CatO = TexMobject("C.O.").set_color(GREEN)
        CatO.scale(0.4)
        CatO.move_to(Lx_2.get_center() + np.array([0, 1.65, 0]))
        
        CatA = TexMobject("C.A.").set_color(BLUE)
        CatA.scale(0.4)
        CatA.move_to(Ly_2.get_center() + np.array([-0.9, 0, 0]))


        sen_2 = TexMobject("sen(\\theta)").set_color(GREEN)
        sen_2.scale(0.4)
        sen_2.move_to(Lx_2.get_center() + np.array([0, 1.65, 0]))
        
        cos_2 = TexMobject("cos(\\theta)").set_color(BLUE)
        cos_2.scale(0.4)
        cos_2.move_to(Ly_2.get_center() + np.array([-0.9, 0, 0]))
         
        self.play(Write(Lx_2), Write(CatO), Write(px_2))
        self.play(Write(Ly_2), Write(CatA), Write(py_2))
        self.play(FadeOut(Lx_2), FadeOut(Ly_2))
        self.wait()

        self.play(
            ReplacementTransform(CatO, sen_2), 
            ReplacementTransform(CatA, cos_2)
        )
        self.wait(2)
        
        # --------------------------------------------------
        
        ec_2 = TexMobject("\\hat{\\theta}"," = ", "-\\hat{i}","sen(\\theta)",
        "+", "\\hat{j}","cos(\\theta)").scale(0.4)
        
        ec_2.move_to(np.array([1.5, 2.5, 0]))
        ec_2[0].set_color(WHITE)
        ec_2[1].set_color(WHITE)
        ec_2[2].set_color(YELLOW)
        ec_2[3].set_color(GREEN)
        ec_2[4].set_color(WHITE)
        ec_2[5].set_color(YELLOW)
        ec_2[6].set_color(BLUE)
        ...
        self.wait()
        self.play(
            Write(ec_2[1]), ReplacementTransform(_i_t.copy(), ec_2[2])
        )
        ...
        r_ec0 = TexMobject(
        "\\vec{r} = r\\hat{r}"
        ).move_to(np.array([0.8, 2.25, 0]))
        
        r_ec1 = TexMobject(
        "= r(\\hat{i}cos(\\theta) + \\hat{j}sen(\\theta) )"
        ).move_to(np.array([1.7, 2.0, 0]))
        
        r_ec2 = TexMobject(
        "= \\hat{i}rcos(\\theta) + \\hat{j}rsen(\\theta)"
        ).move_to(np.array([1.7, 1.75, 0]))
        
        r_ec3 = TexMobject(
        "\\vec{r}", "= (rcos(\\theta), rsen(\\theta))"
        ).move_to(np.array([1.7, 1.5, 0])) 
        ...
        def escribir_ec2(x, y):
            ...
            ec[0].set_color(WHITE)
            ec[2].set_color(YELLOW)
            ec[3].set_color(GREEN)
            ec[4].set_color(GREEN)
            ec[5].set_color(GREEN)
            ec[7].set_color(YELLOW)
            ec[8].set_color(BLUE)
            ec[9].set_color(BLUE)
            ec[10].set_color(BLUE)
        ...
        for i in range(21, 386, 5):
            cruz_cords.append(
            3.51*np.array([np.cos(PI*(i)/180), np.sin(PI*(i)/180), 0])
            )
            
            v_cords3.append(
            np.sqrt(13)*np.array([np.cos(PI*i/180), np.sin(PI*i/180), 0])
            )
            
            v_cords32.append(
            np.sqrt(12.5)*np.array([np.cos(PI*(i-4)/180),
            np.sin(PI*(i-4)/180), 0]))
            
            t_cords3.append(
            np.sqrt(15.7)*np.array([np.cos(PI*(i+28)/180),
            np.sin(PI*(i+28)/180), 0]) )
            
            t_cords32.append(
            np.sqrt(15.5)*np.array([np.cos(PI*(i+28)/180),
            np.sin(PI*(i+28)/180), 0])
            )
        ...
        for i in range(0, len(v_cords3)):
            v_arrows3.append(
            Vector(0.98*v_cords3[i],
            tip_length = 0.2).set_color(RED))
            
            r_arrows3.append(
            Arrow(0.9*v_cords3[i], 1.5*v_cords3[i],
            tip_length = 0.2))

            t_arrows3.append(
            Arrow(v_cords32[i], t_cords3[i],
            tip_length = 0.2))

            cruz_mover.append(
            cruz[i].move_to(cruz_cords[i]))
            

        for i in range(0, len(v_arrows3)):
            v_tex3.append(
                TexMobject("\\vec{r}").move_to(
                    v_arrows3[i].get_center() + np.array([-0.5, 0.2, 0])))
            r_tex3.append(
                TexMobject("\\hat{r}").move_to(
                    1.5*v_arrows3[i].get_end()) )
            t_tex3.append(
                TexMobject("\\hat{\\theta}").move_to(
                    t_cords32[i]))
            i_tex3.append(
                TexMobject("\\hat{i}").move_to(
                    cruz_mover[i].get_center()+ np.array([2.1, -0.2,0])))
            j_tex3.append(
                TexMobject("\\hat{j}").move_to(
                    cruz_mover[i].get_center()+ np.array([0.2, 2.1,0]) ))
        ...
        for i in range(0, len(v_arrows3)):
            if i == 0:
                    ...
                    ReplacementTransform(
                        v_arrows[len(v_arrows)-1], v_arrows3[i]),
                    ReplacementTransform(v_tnn, v_tex3[0]),
                    FadeOut(r_arrows[len(v_arrows)-1]),
                    ReplacementTransform(
                        r_arrows[len(v_arrows)-1], r_arrows3[i].set_color(GRAY)),
                    FadeOut(t_arrows[len(v_arrows)-1]),
                    ReplacementTransform(
                        t_arrows[len(v_arrows)-1], t_arrows3[i].set_color(GRAY)),
                ...
                self.play(
                    ...
                    ReplacementTransform(
                        r_arrows3[i-1], r_arrows3[i].set_color(GRAY)),
                    ReplacementTransform(
                        t_arrows3[i-1], t_arrows3[i].set_color(GRAY)),
                    ReplacementTransform(
                        v_arrows3[i-1], v_arrows3[i].set_color(RED)),
...
class Tres(GraphScene, MovingCameraScene):
    def construct(self):
        ...
        for i in range(35, 385, 5):
            v_cords.append(
                np.sqrt(13)*np.array([np.cos(PI*i/180),
                np.sin(PI*i/180), 0]))
            v_cords2.append(
                np.sqrt(12.5)*np.array([np.cos(PI*(i-4)/180),
                np.sin(PI*(i-4)/180), 0]))
        ...
        for i in range(0, len(v_arrows)):
            v_tex.append(TexMobject("\\vec{r}").move_to(
                v_arrows[i].get_center() + np.array([-0.5, 0.2, 0]) ))
            r_tex.append(TexMobject("\\hat{r}").move_to(
                1.5*v_arrows[i].get_end()) )
            t_tex.append(TexMobject("\\hat{\\theta}").move_to(t_cords2[i]))
